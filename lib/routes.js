...
.post('/quiz/:quiznum', isEtudiant, function(req, res) {
  var numQuizz = req.params.quizznum;
  var etu = req.user.etu_id;
  var rep = req.body;
  dao.reponseSondage(numQuizz, etu, rep, function(bool) {
    if (bool) {
...
.post('/creerSondage', isEnseignant, function(req, res) {
  ...
  dao.insertSondageFull(sondage, function(data) {
    res.send(data);
  });
})
